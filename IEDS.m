function results = IEDS_Secure_Clean()
% IEDS_Secure_Clean
% ------------------------------------------------------------
% Clean, modular, and reproducible implementation of a secure
% image encryption pipeline with integrity (MAC) + full metrics.
%
% Features:
% - Keyed row/column permutation (chaotic, deterministic).
% - Dynamic, invertible S-Box (per image).
% - Multi-round diffusion (forward/backward) with independent keys/IVs.
% - MAC (keyed SHA-256) for integrity.
% - Metrics: Entropy, NPCR, UACI, SSIM, PSNR (encryption/decryption), MSE.
% Save this file as IEDS_Secure_Clean.m and call: IEDS_Secure_Clean
% ------------------------------------------------------------
...
cfg.rounds           = 4;                      % 2..4 (4 يرفع NPCR/UACI/Entropy)
cfg.useDRPE          = true;                   % لقياس Entropy(|E|) على |E|
cfg.showFigs         = true;                   % عرض صور لكل ملف
cfg.writeCSV         = false;                  % حفظ النتائج CSV بجانب البيانات
cfg.csvName          = 'IEDS_metrics.csv';
cfg.workspaceVarName = 'IEDS_results';         % اسم المتغير داخل الـ Workspace
...
for i = 1:n
    ...
    % Context (derives per-image keys deterministically)
    ctx.passphrase = cfg.passphrase;
    ctx.fileId     = files(i).name;
    ctx.rounds     = cfg.rounds;

    % ---- Encrypt ----
    [Ct, macHex] = iedsEncrypt(Iu8, ctx);

    % ---- Decrypt + Verify MAC ----
    [Rec, macOk] = iedsDecrypt(Ct, macHex, ctx); %#ok<NASGU>
    ...
    % ---- NPCR & UACI (flip 1 pixel in input) ----
    If = Iu8; If(1,1) = bitxor(If(1,1), uint8(1));
    Cf = iedsEncrypt(If, ctx);  % التقاط الخرج الأول فقط كصورة مُعمّاة
    ...
    if cfg.showFigs
        showTriptych(Iu8, Ct, Rec, sprintf('IEDS: %s', files(i).name), tern(macOk,'MAC OK','MAC FAIL'));
    end
    ...
    results.PSNR_Encryption(i) = psnrEnc;     % <-- جديد
...
try
    if ~isvarname(cfg.workspaceVarName)
        warning('Invalid workspaceVarName "%s". Using default "IEDS_results".', cfg.workspaceVarName);
        cfg.workspaceVarName = 'IEDS_results';
    end
...
%% ==================== Crypto: Top-Level ===================
function [Ct, macHex] = iedsEncrypt(Iu8, ctx)
% Enc: Permute → S-Box → Multi-Round Diffusion → MAC
    [H, W] = size(Iu8);
    rounds = ctx.rounds;
...
function [Irec, ok] = iedsDecrypt(Ct, macHex, ctx)
% Dec: Verify MAC → Undiffuse → Inverse S-Box → Inverse Perm
    [H, W] = size(Ct);
    rounds  = ctx.rounds;
    keyBytes = sha256Bytes([ctx.passphrase '|' ctx.fileId]);
    ...
    % بناء فهارس عكسية بشكل آمن
    [~, rowIdx] = sort(seq1(1:H), 'ascend');
    invRow = zeros(1,H); invRow(rowIdx) = 1:H;
    [~, colIdx] = sort(seq2(1:W), 'ascend');
    invCol = zeros(1,W); invCol(colIdx) = 1:W;
